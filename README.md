...
#### (Now (1/18/2024) being revised for working with Duck DB dasboarding with Power BI)
...
**Next Steps**  
A critical part of data that is missing is the member ID. While we are able to identify the type of member, having a unique member ID, whether casual or annual member, will be helpful in understanding how often these members are returning to our services, predict churn and retaining them as well. I can use the member ID to detect usage patterns. This can be both casual members or annual members. One can use the member personal information of members, such as age groups, professions, to detect which groups are morelikely to use this service.  
 
**Action Items**
 - Collect user biodata for detailed behavior analysis - student plans and day passes for tourists.
 - Few dates/weeks have particularly high numbers. Collect data to understand of local events will help plan better to serve the customers.
 ...
 - Investigate reasons for the drop in rides started by the users.
